...
echo ''
read -p "Do you want to use Chad's dotfiles? y/n" -n 1 -r
echo    # (optional) move to a new line
if [[ $REPLY =~ ^[Yy]$ ]]
then
    echo ''
	echo "Now pulling down Chad's dotfiles..."
	git clone https://github.com/ChadJessup/dotfiles.git ~/.dotfiles
	echo ''
    ...
    if [[ $? -eq 0 ]]
    then
        echo "Successfully configured your environment with Chad's dotfiles..."
    else
        echo "Chad's dotfiles were not applied successfully..." >&2
fi
else 
	echo ''
    echo "You chose not to apply chad's dotfiles. You will need to configure your environment manually..."
	echo ''
